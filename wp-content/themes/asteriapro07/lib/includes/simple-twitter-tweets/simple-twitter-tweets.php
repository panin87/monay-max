...
class PI_SimpleTwitterTweets extends WP_Widget{
	...
	function PI_SimpleTwitterTweets()  {
		...
		$this->WP_Widget('PI_SimpleTwitterTweets', 'Asteria Twitter Tweets', $widget_ops);
	...
	function register_stt_styles() {
		if(!is_admin()){
			wp_register_style( 'PI_stt_front', get_template_directory_uri().'/lib/includes/simple-twitter-tweets/css/stt.min.css' );
	...
	function PI_load_admin_scripts($hook) {
	    ...
	    // Now load STT JS
		wp_enqueue_script('PI_stt_js', get_template_directory_uri().'/lib/includes/simple-twitter-tweets/js/sttAdmin.min.js' , array('jquery'));
	}
			...
			if($tweets) : ?>
			    <?php foreach($tweets as $t) : ?>
			        <li<?php echo ($showAvatar) ? ' class="avatar"':""; ?><?php echo ($showAvatar && $avatarSize) ? ' style="margin-left:'.($avatarSize+5).'px"':""; ?>>
			        	<?php
			        		if ($showAvatar){
			        			echo '<div class="tweet_avt" style="float:left;"><img ';
